...
| -------------------- | ------------------------------- | ------------------------------------ |
| Architecture         | Démon central (`dockerd`)       | Daemonless, processus éphémères      |
| Privilèges           | Démon root + socket             | Rootless (user namespaces)           |
| Surface d'attaque    | Élevée (socket = contrôle hôte) | Réduite (pas de démon/socket)        |
| Conformité OCI       | Oui                             | Oui (images interopérables)          |
| Containerfile        | Oui                             | Oui + build natif scriptable         |
| Adapté CI rootless   | Nécessite DinD privilégié       | Conçu pour (CI/CD, K8s)              |
